namespace GW2FOX
{
    public static class BossTimings
    {
        ...
        public static List<BossEvent> GetFutureBossEvents()
        {
            ...
            // Entfernen abgelaufener Events
            var currentTime = DateTime.Now.TimeOfDay;
            validEvents.RemoveAll(eventItem => eventItem.Timing < currentTime);
            ...
            // Füge abgeschlossene Events wieder hinten an die Liste an
            var completedEvents = validEvents.Except(resultEvents).ToList();
            Events.RemoveAll(eventItem => completedEvents.Contains(eventItem));
        ...
        public static List<BossEvent> GetNextBossEvents()
        {
            ...
            // Füge abgeschlossene Events wieder hinten an die Liste an
            var completedEvents = validEvents.Except(allBossTimings).ToList();
            Events.RemoveAll(eventItem => completedEvents.Contains(eventItem));
            Events.AddRange(completedEvents);
