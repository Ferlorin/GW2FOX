namespace GW2FOX
{
    partial class Thaida
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Thaida));
            button9 = new Button();
            button8 = new Button();
            button7 = new Button();
            button5 = new Button();
            button4 = new Button();
            button3 = new Button();
            button2 = new Button();
            AttentionThaida = new TextBox();
            Thaidainstance = new TextBox();
            Thaidainfo = new TextBox();
            Back = new Button();
            Symbols = new TextBox();
            Welcome = new TextBox();
            Guild = new TextBox();
            Squadinfo = new TextBox();
            Runinfo = new TextBox();
            pictureBox5 = new PictureBox();
            ((System.ComponentModel.ISupportInitialize)pictureBox5).BeginInit();
            SuspendLayout();
            // 
            // button9
            // 
            button9.BackgroundImage = (Image)resources.GetObject("button9.BackgroundImage");
            button9.BackgroundImageLayout = ImageLayout.Center;
            button9.Cursor = Cursors.Cross;
            button9.FlatStyle = FlatStyle.Flat;
            button9.Location = new Point(371, 430);
            button9.Name = "button9";
            button9.Size = new Size(75, 23);
            button9.TabIndex = 115;
            button9.UseVisualStyleBackColor = true;
            button9.Click += Attentionthaida_Click;
            // 
            // button8
            // 
            button8.BackgroundImage = (Image)resources.GetObject("button8.BackgroundImage");
            button8.BackgroundImageLayout = ImageLayout.Center;
            button8.Cursor = Cursors.Cross;
            button8.FlatStyle = FlatStyle.Flat;
            button8.Location = new Point(371, 385);
            button8.Name = "button8";
            button8.Size = new Size(75, 23);
            button8.TabIndex = 114;
            button8.UseVisualStyleBackColor = true;
            button8.Click += Thaidainstance_Click;
            // 
            // button7
            // 
            button7.BackgroundImage = (Image)resources.GetObject("button7.BackgroundImage");
            button7.BackgroundImageLayout = ImageLayout.Center;
            button7.Cursor = Cursors.Cross;
            button7.FlatStyle = FlatStyle.Flat;
            button7.Location = new Point(371, 340);
            button7.Name = "button7";
            button7.Size = new Size(75, 23);
            button7.TabIndex = 113;
            button7.UseVisualStyleBackColor = true;
            button7.Click += Thaidainfo_Click;
            // 
            // button5
            // 
            button5.BackgroundImage = (Image)resources.GetObject("button5.BackgroundImage");
            button5.BackgroundImageLayout = ImageLayout.Center;
            button5.Cursor = Cursors.Cross;
            button5.FlatStyle = FlatStyle.Flat;
            button5.Location = new Point(1796, 295);
            button5.Name = "button5";
            button5.Size = new Size(75, 23);
            button5.TabIndex = 112;
            button5.UseVisualStyleBackColor = true;
            button5.Click += Welcome_Click;
            // 
            // button4
            // 
            button4.BackgroundImage = (Image)resources.GetObject("button4.BackgroundImage");
            button4.BackgroundImageLayout = ImageLayout.Center;
            button4.Cursor = Cursors.Cross;
            button4.FlatStyle = FlatStyle.Flat;
            button4.Location = new Point(1321, 295);
            button4.Name = "button4";
            button4.Size = new Size(75, 23);
            button4.TabIndex = 111;
            button4.UseVisualStyleBackColor = true;
            button4.Click += Guild_Click;
            // 
            // button3
            // 
            button3.BackgroundImage = (Image)resources.GetObject("button3.BackgroundImage");
            button3.BackgroundImageLayout = ImageLayout.Center;
            button3.Cursor = Cursors.Cross;
            button3.FlatStyle = FlatStyle.Flat;
            button3.Location = new Point(846, 295);
            button3.Name = "button3";
            button3.Size = new Size(75, 23);
            button3.TabIndex = 110;
            button3.UseVisualStyleBackColor = true;
            button3.Click += Squadinfo_Click;
            // 
            // button2
            // 
            button2.BackgroundImage = (Image)resources.GetObject("button2.BackgroundImage");
            button2.BackgroundImageLayout = ImageLayout.Center;
            button2.Cursor = Cursors.Cross;
            button2.FlatStyle = FlatStyle.Flat;
            button2.Location = new Point(371, 295);
            button2.Name = "button2";
            button2.Size = new Size(75, 23);
            button2.TabIndex = 109;
            button2.UseVisualStyleBackColor = true;
            button2.Click += Runinfo_Click;
            // 
            // AttentionThaida
            // 
            AttentionThaida.Font = new Font("Segoe UI", 9F);
            AttentionThaida.Location = new Point(15, 414);
            AttentionThaida.Multiline = true;
            AttentionThaida.Name = "AttentionThaida";
            AttentionThaida.ReadOnly = true;
            AttentionThaida.ScrollBars = ScrollBars.Vertical;
            AttentionThaida.Size = new Size(350, 39);
            AttentionThaida.TabIndex = 107;
            AttentionThaida.Text = "≪ATTENTION≫ Meta -Train │ ☠Lvl 50 │ “Admiral Taidha Covington” │ ☣[&BKgBAAA=]☣ - Join us!\r\n\r\n";
            // 
            // Thaidainstance
            // 
            Thaidainstance.Font = new Font("Segoe UI", 9F);
            Thaidainstance.Location = new Point(15, 369);
            Thaidainstance.Multiline = true;
            Thaidainstance.Name = "Thaidainstance";
            Thaidainstance.ReadOnly = true;
            Thaidainstance.ScrollBars = ScrollBars.Vertical;
            Thaidainstance.Size = new Size(350, 39);
            Thaidainstance.TabIndex = 106;
            Thaidainstance.Text = "Go out of fight, then right click group 2 & join ☣Bloodtide Coast☣\r\n\r\n";
            // 
            // Thaidainfo
            // 
            Thaidainfo.BackColor = SystemColors.Highlight;
            Thaidainfo.Font = new Font("Segoe UI", 9F);
            Thaidainfo.Location = new Point(15, 324);
            Thaidainfo.Multiline = true;
            Thaidainfo.Name = "Thaidainfo";
            Thaidainfo.ReadOnly = true;
            Thaidainfo.ScrollBars = ScrollBars.Vertical;
            Thaidainfo.Size = new Size(350, 39);
            Thaidainfo.TabIndex = 105;
            Thaidainfo.Text = "☠Lvl 50 ≪Admiral Taidha Covington≫ │ ☣[&BKgBAAA=]☣\r\n\r\n";
            // 
            // Back
            // 
            Back.BackgroundImage = (Image)resources.GetObject("Back.BackgroundImage");
            Back.BackgroundImageLayout = ImageLayout.Center;
            Back.Cursor = Cursors.Cross;
            Back.FlatStyle = FlatStyle.Flat;
            Back.Location = new Point(1864, 1002);
            Back.Name = "Back";
            Back.Size = new Size(40, 40);
            Back.TabIndex = 104;
            Back.UseVisualStyleBackColor = true;
            Back.Click += Back_Click;
            // 
            // Symbols
            // 
            Symbols.Font = new Font("Segoe UI", 11F);
            Symbols.Location = new Point(1458, 12);
            Symbols.Multiline = true;
            Symbols.Name = "Symbols";
            Symbols.ReadOnly = true;
            Symbols.ScrollBars = ScrollBars.Vertical;
            Symbols.Size = new Size(429, 85);
            Symbols.TabIndex = 103;
            // 
            // Welcome
            // 
            Welcome.Font = new Font("Segoe UI", 9F);
            Welcome.Location = new Point(1440, 268);
            Welcome.Multiline = true;
            Welcome.Name = "Welcome";
            Welcome.ReadOnly = true;
            Welcome.ScrollBars = ScrollBars.Vertical;
            Welcome.Size = new Size(350, 50);
            Welcome.TabIndex = 102;
            // 
            // Guild
            // 
            Guild.Font = new Font("Segoe UI", 9F);
            Guild.Location = new Point(965, 268);
            Guild.Multiline = true;
            Guild.Name = "Guild";
            Guild.ReadOnly = true;
            Guild.ScrollBars = ScrollBars.Vertical;
            Guild.Size = new Size(350, 50);
            Guild.TabIndex = 101;
            // 
            // Squadinfo
            // 
            Squadinfo.Font = new Font("Segoe UI", 9F);
            Squadinfo.Location = new Point(490, 268);
            Squadinfo.Multiline = true;
            Squadinfo.Name = "Squadinfo";
            Squadinfo.ReadOnly = true;
            Squadinfo.ScrollBars = ScrollBars.Vertical;
            Squadinfo.Size = new Size(350, 50);
            Squadinfo.TabIndex = 100;
            Squadinfo.Text = "\r\n";
            // 
            // Runinfo
            // 
            Runinfo.Font = new Font("Segoe UI", 9F);
            Runinfo.Location = new Point(15, 268);
            Runinfo.Multiline = true;
            Runinfo.Name = "Runinfo";
            Runinfo.ReadOnly = true;
            Runinfo.ScrollBars = ScrollBars.Vertical;
            Runinfo.Size = new Size(350, 50);
            Runinfo.TabIndex = 99;
            // 
            // pictureBox5
            // 
            pictureBox5.Image = Properties.Resources.WBsheader;
            pictureBox5.Location = new Point(12, 232);
            pictureBox5.Name = "pictureBox5";
            pictureBox5.Size = new Size(1880, 30);
            pictureBox5.TabIndex = 126;
            pictureBox5.TabStop = false;
            // 
            // Thaida
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(1904, 1041);
            Controls.Add(pictureBox5);
            Controls.Add(button9);
            Controls.Add(button8);
            Controls.Add(button7);
            Controls.Add(button5);
            Controls.Add(button4);
            Controls.Add(button3);
            Controls.Add(button2);
            Controls.Add(AttentionThaida);
            Controls.Add(Thaidainstance);
            Controls.Add(Thaidainfo);
            Controls.Add(Back);
            Controls.Add(Symbols);
            Controls.Add(Welcome);
            Controls.Add(Guild);
            Controls.Add(Squadinfo);
            Controls.Add(Runinfo);
            Icon = (Icon)resources.GetObject("$this.Icon");
            Name = "Thaida";
            Text = "GW2FOX";
            WindowState = FormWindowState.Maximized;
            ((System.ComponentModel.ISupportInitialize)pictureBox5).EndInit();
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion
        private Button button9;
        private Button button8;
        private Button button7;
        private Button button5;
        private Button button4;
        private Button button3;
        private Button button2;
        private TextBox AttentionThaida;
        private TextBox Thaidainstance;
        private TextBox Thaidainfo;
        private Button Back;
        private TextBox Symbols;
        private TextBox Welcome;
        private TextBox Guild;
        private TextBox Squadinfo;
        private TextBox Runinfo;
        private PictureBox pictureBox5;
    }
}