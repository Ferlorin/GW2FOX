...
namespace GW2FOX
{
    public partial class DredgeComissar : BaseForm
    {
        public DredgeComissar()
        {
            InitializeComponent();
        ...
        private async Task LoadItemPriceInformation()
        {
            try
            {
                ...
                {
                    string apiUrl = "https://api.guildwars2.com/v2/items/39468";
        ...
        private async Task<int> GetItemPriceCopper()
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    string jsonResult = await client.GetStringAsync("https://api.guildwars2.com/v2/commerce/prices/39468");
                    JObject resultObject = JObject.Parse(jsonResult);
